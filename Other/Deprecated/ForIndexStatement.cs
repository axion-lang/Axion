...
namespace Axion.Core.Processing.Syntactic.Statements {
    ...
    public class ForIndexStatement : LoopStatement {
        ...
        /// </summary>
        internal ForIndexStatement(AstNode parent) : base(parent) {
            MarkStartAndEat(KeywordFor);

            if (!MaybeEat(Semicolon)) {
                InitStmt = Expression.ParseExpr(parent);
                Eat(Semicolon);
            }
            ...
            if (!MaybeEat(Spec.NeverExprStartTypes)) {
                IterStmt = Expression.ParseInfixExpr(this);
            }
