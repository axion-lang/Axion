    ...
    public partial class SyntaxParser {
        ...
        private Statement ParseDecorated() {
            ...
            if (stmt is IDecorated def) {
                ...
                unit.ReportError(Spec.ERR_InvalidDecoratorPosition, stmt);
            }
        ...
        private TypeName ParseTypeName() {
            ...
            // tuples
                ...
                leftTypeName = types.Count == 1
                    ? types[0]
                    : new TupleTypeName(types.ToArray(), start, stream.Token);
            }
            ...
            if (leftTypeName != null) {
                ...
                if (stream.MaybeEat(OpLessThan)) {
                    ...
                    leftTypeName = new GenericTypeName(
                        leftTypeName,
                        generics.ToArray(),
                        stream.Token
                    );
                ...
                if (stream.MaybeEat(LeftBracket)) {
                    ...
                    leftTypeName = new ArrayTypeName(leftTypeName, stream.Token);
                }
                ...
                if (stream.MaybeEat(RightParenthesis)) {
                    ...
                    do {
                        ...
                        if (stream.MaybeEat(Identifier)) {
                            var id = (IdentifierToken) stream.Token;
                            if (stream.MaybeEat(Assign)) {
                                if (!allowNamed) {
                                    unit.ReportError("Base of this type cannot be named.", id);
                                }
        ...
        private EnumDefinition ParseEnumDef() {
            ...
            if (name == null) {
                return new EnumDefinition(start, stream.Token, null);
            }
        ...
        private EnumItem ParseEnumItem() {
            ...
            if (stream.MaybeEat(Assign)) {
                ...
                if (value == null) {
                    unit.Blame(BlameType.ConstantValueExpected, stream.Token);
                }
            }
        ...
        private ClassDefinition ParseClassDef() {
            ...
            foreach ((NameExpression typeLabel, TypeName type) in types) {
                if (typeLabel == null) {
                    ...
                    keywords.Add(type);
                    if (typeLabel.Name.Value == "metaclass") {
                        metaClass = type;
                    }
                }
            }
            ...
            classDef.MarkPosition(start, stream.Token);
            return classDef;
        }
        ...
        private FunctionDefinition ParseFunctionDef() {
            Token          start         = StartExprOrStmt(KeywordFn);
            Expression     fullName      = ParseQualifiedName();
            NameExpression funcName      = null;
            NameExpression interfaceName = null;
            if (fullName is MemberExpression m) {
                funcName = m.Member;
                if (m.Target is NameExpression name) {
                    interfaceName = name;
                }
                else {
                    unit.ReportError(
            ...
            else if (fullName is NameExpression n) {
                funcName = n;
            }
            else {
                unit.ReportError(
            ...
            var ret = new FunctionDefinition(
                funcName.Name.Value,
                interfaceName,
                parameters,
        ...
        private Parameter[] ParseFunctionParameterList(TokenType terminator) {
            ...
            if (readMultiply
                ...
                unit.ReportError("named arguments must follow bare *", stream.Token);
            }
        ...
        private Parameter ParseNamedParameter(
            ...
            if (stream.MaybeEat(Assign)) {
                ...
                unit.Blame(BlameType.ExpectedDefaultParameterValue, stream.Token);
            }
