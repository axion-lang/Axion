...
namespace Axion.Core.Processing.Lexical.Lexer {
    internal static class LiteralParser {
        ...
        private static double ParseFloatNoCatch(string text) {
            ...
            switch (s.ToUpper(CultureInfo.InvariantCulture).TrimStart()) {
                ...
                default: {
                    ...
                    return res == 0.0 && text.TrimStart().StartsWith("-") ? 0.0 : res; //-V3024
                }
            }
        }
        ...
        private static int CharValue(char ch, int b) {
            HexValue(ch, out int val, out BlameType _);
            if (val >= b) {
        ...
        internal static void HexValue(char ch, out int result, out BlameType errorType) {
            errorType = BlameType.None;
            if (!HexValue(ch, out result)) {
                errorType = BlameType.BadCharacterForIntegerValue;
            }
        }
    }
}