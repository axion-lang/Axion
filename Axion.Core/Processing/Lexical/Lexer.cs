...
public class Lexer {
    ...
    public HashSet<TokenType> ProcessTerminators { get; } = new();

    public Lexer(Unit unit) {
        this.unit = unit;
        stream    = this.unit.TextStream;
        ProcessTerminators.Add(End);
    }
    ...
    Token? ReadWhite() {
        ...
        if (unit.TokenStream[^1].Type != Newline
         || ln.IsWhiteSpace()
        ...
        // newline-operator-continuation check
        foreach (var op in OperatorsKeys) {
            if (ln.StartsWith(op)
             && ln.Length > op.Length
             && !char.IsLetterOrDigit(ln[op.Length..][^1])) {
                unit.TokenStream[^1].EndingWhite += value;
                return null;
            }
        }
        ...
        if (!consistent) {
            LanguageReport.To(
                ...
                new ErrorSpan {
                    Unit  = unit,
                    Start = inconsistencyStart,
                    End   = stream.Location - (0, 1)
                }
            );
        }
        else {
    ...
    StringInterpolation ReadStringInterpolation() {
        ...
        lexer.ProcessTerminators.Add(CloseBrace);
        while (true) {
    ...
    void ReadEscapeSeq() {
        ...
        // \u h{4} or \U h{8}
        else if (!stream.Eat("u", "U").IsEmpty) {
            var u = stream.Char;
            raw += u;
