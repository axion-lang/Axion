...
public sealed class FunctionParameter : NameDef {
    ...
    ///     </code>
    /// </summary>
    public static NodeList<FunctionParameter, Ast> ParseList(
        Node               parent,
        params TokenType[] terminators
    ) {
        var s = parent.Unit.TokenStream;

        var parameters = new NodeList<FunctionParameter, Ast>(parent);
        ...
        while (!s.PeekIs(terminators)) {
            ...
            if (s.MaybeEat(Star)) {
                if (haveMultiply) {
                    ...
                    return new NodeList<FunctionParameter, Ast>(parent);
                }
