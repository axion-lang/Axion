...
public partial class ScopeExpr : Node {
    ...
    public List<(T item, ScopeExpr parentScope, int index)> FindItemsOfType<T>() {
        return FindItemsOfType(
            new List<(
                T item,
                ScopeExpr itemParentScope,
                int itemIndex
                )>()
        );
    }

    List<(T item, ScopeExpr parentScope, int index)> FindItemsOfType<T>(
        List<(T item, ScopeExpr parentScope, int index)> results
    ) {
        for (var i = 0; i < Items.Count; i++) {
            var item = Items[i];
            if (item is T expr) {
                results.Add((expr, this, i));
            }
            else {
    ...
    public ScopeExpr Parse() {
        ...
        else if (hasNewline) // newline followed by not indent or '{'
        {
            LanguageReport.To(
